                ...
                <div class="box">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools float-right">
                            <a href="{{ route('restaurant-admin.verification.expiry-report') }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-body text-center py-5">
                    ...
                    <div id="confirmDetails" class="alert alert-info mb-4" style="display: none;">
                    </div>
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="updateDocumentStatus()">Update
                        Status</button>
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        ...
        function changeStatus(documentId, status) {
            ...
            // Show confirmation modal based on status
            const confirmConfig = {
                'approved': {
                    title: 'Approve Document?',
                    message: 'Are you sure you want to approve this document? You can add notes before confirming.',
                    icon: 'fa-check-circle',
                    btnColor: 'btn-success'
                },
                'rejected': {
                    title: 'Reject Document?',
                    message: 'Are you sure you want to reject this document? You should provide notes explaining the rejection.',
                    icon: 'fa-times-circle',
                    btnColor: 'btn-danger'
                }
            };

            const config = confirmConfig[status];
            if (config) {
                $('#confirmTitle').text(config.title);
                $('#confirmMessage').html(config.message);
                $('#confirmIcon').removeClass().addClass(`fa ${config.icon}`);
                $('#confirmActionBtn').removeClass().addClass(`btn ${config.btnColor}`).html(
                    `<i class="fa fa-check me-2"></i>Yes, ${status === 'approved' ? 'Approve' : 'Reject'}`);

                const iconWrapper = $('#confirmIconWrapper');
                iconWrapper.removeClass('bg-success bg-danger bg-warning');
                if (config.btnColor === 'btn-success') {
                    iconWrapper.addClass('bg-success');
                } else if (config.btnColor === 'btn-danger') {
                    iconWrapper.addClass('bg-danger');
                }

                // Set callback to open status modal
        ...
        function performBulkAction() {
            ...
            showConfirmation(action, selectedIds.length, function() {
        ...
        function showConfirmation(action, count, callback) {
            ...
            let title, message, icon, btnClass, btnColor;

            const actionConfig = {
                'approve': {
                    title: 'Approve Documents?',
                    message: `You are about to approve <strong>${count}</strong> document(s). This action cannot be undone.`,
                    icon: 'fa-check-circle',
                    btnColor: 'btn-success',
                    btnText: 'Yes, Approve'
                },
                'reject': {
                    ...
                    message: `You are about to reject <strong>${count}</strong> document(s). This action cannot be undone.`,
                    icon: 'fa-times-circle',
                    btnColor: 'btn-danger',
                    btnText: 'Yes, Reject'
                },
                'mark_urgent': {
                    ...
                    message: `You are about to mark <strong>${count}</strong> document(s) as urgent.`,
                    icon: 'fa-exclamation-triangle',
                    btnColor: 'btn-warning',
                    btnText: 'Yes, Mark Urgent'
                }
            };
            ...
            $('#confirmIcon').removeClass().addClass(`fa ${config.icon}`);
            $('#confirmActionBtn').removeClass().addClass(`btn ${config.btnColor}`).html(
                `<i class="fa fa-check me-2"></i>${config.btnText}`);

            // Update icon wrapper background color
            const iconWrapper = $('#confirmIconWrapper');
            iconWrapper.removeClass('bg-success bg-danger bg-warning');
            if (config.btnColor === 'btn-success') {
                iconWrapper.addClass('bg-success');
                ...
                iconWrapper.addClass('bg-danger');
            } else if (config.btnColor === 'btn-warning') {
                iconWrapper.addClass('bg-warning');
            }
