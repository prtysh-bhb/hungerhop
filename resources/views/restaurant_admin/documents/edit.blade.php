            ...
            <div class="col-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('restaurant-admin.documents.update', $document->id) }}" method="POST"
                            ...
                            <div class="row">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if ($document->status === 'approved')
                                                                    <span
                                                                        class="badge badge-success">{{ ucfirst($document->status) }}</span>
                                                                @elseif($document->status === 'rejected')
                                                                    <span
                                                                        class="badge badge-danger">{{ ucfirst($document->status) }}</span>
                                                                @else
                                                                    <span
                                                                        class="badge badge-warning">{{ ucfirst($document->status) }}</span>
                                                                @endif
                                                            </td>
                                                        </tr>
                                                        <tr>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="document_type" name="document_type" required>
                                            @foreach ($documentTypes as $key => $type)
                                                <option value="{{ $key }}"
                                                    {{ old('document_type', $document->document_type) == $key ? 'selected' : '' }}>
                                                    {{ $type }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="file" class="form-control-file" id="document_file"
                                            name="document_file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx">
                                        <small class="form-text text-muted">
                                            Leave empty to keep current file. Supported formats: PDF, JPG, PNG, DOC, DOCX.
                                            Maximum size: 10MB
                                        </small>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <small class="form-text text-muted">Leave empty if document doesn't expire. Maximum
                                            10 years from today.</small>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group" id="rejection-reason-group"
                                        style="{{ old('status', $document->status) == 'rejected' ? '' : 'display: none;' }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            function validateDateYear(value) {
                ...
                const minYear = currentYear; // Cannot be in the past
                const maxYear = currentYear + 10; // Maximum 10 years from now
            ...
            function validateExpiryDate() {
                const input = $('#expires_at');
                const value = input.val();
                const errorDiv = $('#expires_at_error');
                ...
                if (!yearValidation.isValid) {
                    ...
                    errorDiv.text(yearValidation.message).show();
                    return false;
                }
                ...
                if (selectedDate < tomorrow) {
                    input.addClass('is-invalid');
                    errorDiv.text('Expiry date must be at least tomorrow').show();
                    return false;
                }
            ...
            // Add expiry date validation on change and input
            $('#expires_at').on('change input', function() {
                validateExpiryDate();
            });
