...
@section('content')
    ...
    @php
        ...
        $paymentRequired = $tenant ? $tenant->isPaymentRequired() : true;
        $userPendingApproval = $user && $user->status === 'pending_approval';
    @endphp

    @if ($paymentRequired || $userPendingApproval)
        <div class="alert alert-warning alert-dismissible fade show" role="alert"
            style="margin: 20px; border-left: 4px solid #ff6b6b;">
            <div class="d-flex align-items-center">
                ...
                <div class="flex-grow-1">
                    ...
                    <p class="mb-2">
                        @if ($userPendingApproval)
                            Your account is pending activation. Complete your subscription payment to activate your account
                            and access all platform features.
                        @else
                            Your subscription is not active. To access all features and start managing your restaurants,
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @if ($tenant)
                            <span class="badge badge-info">
                                ...
                                ₹{{ number_format((float) $tenant->calculateSubscriptionAmount()) }}/month
                            </span>
                        @endif
                    </div>
                </div>
        ...
        <div class="alert alert-info" style="margin: 20px;">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    @if ($userPendingApproval)
                        You can view this dashboard but cannot access other features until account activation is completed
                        through payment.
                    @else
                        You can view this dashboard but cannot access other features until payment is completed.
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="margin: 20px;">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <a href="{{ route('admin.tenant.payment.plans') }}" class="btn btn-sm btn-danger ms-2">
    ...
    <section class="content">
        @if ($paymentRequired || $userPendingApproval)
            <!-- Dashboard Content Overlay for Payment Required -->
            <div class="position-relative">
                <div
                    style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(255,255,255,0.8); z-index: 10; display: flex; align-items: center; justify-content: center;">
                    <div class="text-center">
                        ...
                        <p class="text-muted">
                            ...
                            @else
                                Complete payment to access dashboard features
                            @endif
                        </p>
                        <a href="{{ route('admin.tenant.payment.plans') }}" class="btn btn-warning">
                            <i class="fa fa-credit-card me-1"></i>
                            @if ($userPendingApproval)
                                Activate Account
                            @else
                ...
                <div style="filter: blur(2px); pointer-events: none;">
        @endif
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                <h2 class="my-0 fw-700" id="total-orders">{{ $dashboardData['totalOrders'] ?? 89 }}</h2>
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                <h2 class="my-0 fw-700" id="completed-orders">{{ $dashboardData['totalDelivered'] ?? 899 }}
                                </h2>
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                <h2 class="my-0 fw-700" id="canceled-orders">{{ $dashboardData['totalCanceled'] ?? 59 }}
                                </h2>
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                <h2 class="my-0 fw-700" id="total-revenue">
                                    @php
                                        $revenue = $dashboardData['totalRevenue'] ?? 0;
                                        if ($revenue >= 1000) {
                                            echo '$' . number_format($revenue / 1000) . 'K';
                                        } else {
                ...
                <div class="box bg-transparent no-shadow">
                    ...
                    <div class="box-body px-0">
                        <div class="review-slider owl-carousel">
                            @if (isset($dashboardData['reviews']) && $dashboardData['reviews']->count() > 0)
                                                        ...
                                                        <div>
                                                            <p class="mb-0">
                                                                @if ($review->customer && $review->customer->user)
                                                                    {{ $review->customer->user->first_name }}
...
@section('scripts')
    ...
    <script>
        ...
        function updateDashboardStats() {
            fetch('{{ route('admin.dashboard.tenant.stats') }}')
                .then(response => response.json())
                .then(data => {
                    console.log('Dashboard data received:', data);
                    updateCounter(document.getElementById('total-orders'), data.totalOrders);
                    updateCounter(document.getElementById('completed-orders'), data.completedOrders);
                    updateCounter(document.getElementById('canceled-orders'), data.canceledOrders);
                    // Format revenue to match a reasonable display
                    let formattedRevenue;
                    if (data.totalRevenue >= 1000) {
                        // For values 1000 or more, show in K format (e.g., $1K, $2K)
                        formattedRevenue = '$' + Math.round(data.totalRevenue / 1000) + 'K';
                    } else {
                        // For values less than 1000, show actual amount (e.g., $296)
                        formattedRevenue = '$' + Math.round(data.totalRevenue);
                    }
                    console.log('Formatted revenue:', formattedRevenue, 'from raw:', data.totalRevenue);
                    updateCounter(document.getElementById('total-revenue'), formattedRevenue);
                })
                .catch(error => {
