                    ...
                    <div class="d-inline-block align-items-center">
                        <nav>
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('admin.tenant.payment.plans') }}">Payment</a>
                                </li>
                    ...
                    <div class="box">
                        <div class="box-body text-center">
                            <h3 class="text-primary">
                                ₹{{ number_format($payments->where('status', 'completed')->sum('total_amount'), 2) }}</h3>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                @if ($payment->gateway_transaction_id)
                                                    <br><small
                                                        class="text-muted">{{ Str::limit($payment->gateway_transaction_id, 20) }}</small>
                                                @endif
                                            </td>
                                            <td>
                                                <span
                                                    class="badge bg-info">{{ ucfirst($payment->subscription_plan) }}</span>
                                                <br><small class="text-muted">{{ $payment->restaurant_count }}
                                                    Restaurant(s)</small>
                                            </td>
                                            <td>
                                                <strong
                                                    class="text-primary">₹{{ number_format($payment->total_amount, 2) }}</strong>
                                                <br><small class="text-muted">Base:
                                                    ₹{{ number_format($payment->base_amount, 2) }}</small>
                                            </td>
                                            <td>
                                                @php
                                                ...
                                                @endphp
                                                <span
                                                    class="badge bg-{{ $statusClass }}">{{ ucfirst($payment->status) }}</span>
                                                @if ($payment->failure_reason)
                                                    <br><small
                                                        class="text-danger">{{ Str::limit($payment->failure_reason, 30) }}</small>
                                                @endif
                                            </td>
                                            <td>
                                                <span
                                                ...
                                                <br><small
                                                    class="text-muted">{{ ucfirst($payment->payment_gateway) }}</small>
                                            </td>
                                            <td>
                                                <small>
                                                    <strong>From:</strong>
                                                    {{ $payment->billing_period_start->format('M d, Y') }}<br>
                                                    <strong>To:</strong>
                                                    {{ $payment->billing_period_end->format('M d, Y') }}
                                                </small>
                                            </td>
                                            <td>
                                                <small>
