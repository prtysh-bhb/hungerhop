                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="subscription_plan" name="subscription_plan">
                                            ...
                                            <option value="LITE"
                                                {{ request('subscription_plan') == 'LITE' ? 'selected' : '' }}>Lite
                                            </option>
                                            <option value="PLUS"
                                                {{ request('subscription_plan') == 'PLUS' ? 'selected' : '' }}>Plus
                                            </option>
                                            <option value="PRO_MAX"
                                                {{ request('subscription_plan') == 'PRO_MAX' ? 'selected' : '' }}>Pro Max
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item"
                                                href="{{ route('admin.tenants.export.excel', request()->query()) }}">
                                                <i class="fa fa-file-excel-o"></i> Excel (CSV)
                                            </a></li>
                                        <li><a class="dropdown-item"
                                                href="{{ route('admin.tenants.export.pdf', request()->query()) }}"
                                                target="_blank">
                                        ...
                                        <tbody>
                                            @foreach ($tenants as $tenant)
                                                <tr>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <div>
                                                                ...
                                                                <small
                                                                    class="text-muted">{{ $tenant->contact_person }}</small>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div>
                                                            <small class="d-block"><i class="fa fa-envelope"></i>
                                                                {{ $tenant->email }}</small>
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge badge-pill badge-info">{{ $tenant->subscription_plan }}</span>
                                                        <small
                                                            class="d-block text-muted">${{ number_format($tenant->monthly_base_fee, 2) }}/mo</small>
                                                    </td>
                                                    <td>
                                                        <span class="fw-bold">{{ $tenant->restaurants()->count() }}</span>
                                                        / {{ $tenant->total_restaurants }}
                                                        <div class="progress mt-1" style="height: 4px;">
                                                            <div class="progress-bar bg-primary"
                                                                style="width: {{ $tenant->total_restaurants > 0 ? ($tenant->restaurants()->count() / $tenant->total_restaurants) * 100 : 0 }}%">
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        @switch($tenant->status)
                                                            @case('approved')
                                                                <span
                                                                    class="badge badge-pill badge-success status-badge">Approved</span>
                                                            @break

                                                            @case('pending')
                                                                <span
                                                                    class="badge badge-pill badge-warning status-badge">Pending</span>
                                                            @break

                                                            @case('suspended')
                                                                <span
                                                                    class="badge badge-pill badge-danger status-badge">Suspended</span>
                                                            @break

                                                            @case('rejected')
                                                                <span
                                                                    class="badge badge-pill badge-dark status-badge">Rejected</span>
                                                            @break

                                                            @default
                                                                <span
                                                                    class="badge badge-pill badge-secondary status-badge">{{ ucfirst($tenant->status) }}</span>
                                                        @endswitch
                                                    </td>
                                                    <td>
                                                        <small
                                                            class="text-muted">{{ $tenant->created_at->format('M d, Y') }}</small>
                                                    </td>
                                                    <td>
                                                        <div class="btn-group">
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-info"
                                                                title="Update Status"
                                                                onclick="updateStatus({{ $tenant->id }})">
