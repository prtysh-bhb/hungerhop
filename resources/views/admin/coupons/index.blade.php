...
<div class="container">
    ...
    @if(!Route::has('admin.coupons.toggle'))
        <div class="alert alert-warning">
            ...
            <br>Add this to your routes: <code>Route::post('/admin/coupons/{coupon}/toggle', [CouponController::class, 'toggle'])->name('admin.coupons.toggle');</code>
        </div>
    @endif
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-2">
                                <form method="POST" action="{{ route('admin.coupons.toggle', $coupon->id) }}" class="m-0 toggle-coupon-form" data-coupon-id="{{ $coupon->id }}">
                                    @csrf
                                    <label class="switch mb-0" style="vertical-align: middle;">
                                        <input type="checkbox" class="toggle-coupon-switch" data-coupon-id="{{ $coupon->id }}" {{ $coupon->is_active ? 'checked' : '' }}>
                                        <span class="switch-indicator"></span>
                                    </label>
                                </form>
                                <span class="badge {{ $coupon->is_active ? 'bg-success' : 'bg-secondary' }} status-badge" id="status-badge-{{ $coupon->id }}">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.toggle-coupon-switch').forEach(function(switchEl) {
            switchEl.addEventListener('change', function(e) {
                ...
                const form = document.querySelector('.toggle-coupon-form[data-coupon-id="' + couponId + '"]');
