...
class PaymentController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        // Check if there's a pending upgrade payment
        $pendingPayment = $tenant->subscriptionPayments()
            ->where('status', SubscriptionPayment::STATUS_PENDING)
            ->latest()
            ->first();
        ...
        if ($pendingPayment) {
            // Use the pending payment amount (which is the upgrade cost)
            $subscriptionAmount = $pendingPayment->total_amount;
            $planLimits = $tenant->getPlanLimits($pendingPayment->subscription_plan);
            $isUpgradePayment = true;
        } else {
            // Use regular subscription amount
            $subscriptionAmount = $tenant->calculateSubscriptionAmount();
            $planLimits = $tenant->getPlanLimits();
            $isUpgradePayment = false;
        }

        return view('pages.tenant_admin.payment.checkout', compact('tenant', 'billingPeriod', 'subscriptionAmount', 'planLimits', 'pendingPayment', 'isUpgradePayment'));
    }
    ...
    private function createStripePayment($payment, $requestData = [])
    {
        ...
        try {
            ...
            // TEMPORARY: Comment out actual Stripe API call for testing
            // $paymentIntent = \Stripe\PaymentIntent::create($paymentIntentData);

            // TEMPORARY: Simulate successful PaymentIntent response for testing
            $paymentIntent = (object) [
                'id' => 'pi_test_'.uniqid(),
                'amount' => $amountInPaisa,
                'status' => 'requires_payment_method',
                'currency' => 'inr',
                'client_secret' => 'pi_test_'.uniqid().'_secret_test_key',
            ];

            \Log::info('Stripe PaymentIntent created successfully (SIMULATED)', [
    ...
    public function history(Request $request)
    {
        ...
        $query = $tenant->subscriptionPayments();
    ...
    public function updatePlan(Request $request)
    {
        ...
        if (! $tenant) {
            return redirect()->route('admin.tenant.payment.plans')->with('error', 'Tenant not found.');
        }

        $validated = $request->validate([
            'subscription_plan' => 'required|in:LITE,PLUS,PRO_MAX',
        ]);

        $newPlan = $validated['subscription_plan'];

        // Get the plan limits to automatically assign the maximum restaurants for this plan
        $newLimits = $tenant->getPlanLimits($newPlan);
        if (! $newLimits) {
            return redirect()->route('admin.tenant.payment.plans')
                ->with('error', 'Invalid plan selected.');
        }

        // Automatically set restaurant count to the plan's maximum
        $newRestaurantCount = $newLimits['max_restaurants'];
        $currentPlan = $tenant->subscription_plan;

        // Check if it's the same plan
        if ($newPlan === $currentPlan) {
            return redirect()->route('admin.tenant.payment.plans')
                ->with('error', 'You have selected the same plan. Please choose a different plan.');
        }

        // Get plan limits using the tenant method
        $currentLimits = $tenant->getPlanLimits($currentPlan);

        // Define plan hierarchy for comparison (higher index = higher tier)
        $planHierarchy = ['LITE' => 1, 'PLUS' => 2, 'PRO_MAX' => 3];
        $currentPlanTier = $planHierarchy[$currentPlan] ?? 0; // Default to 0 for null/unknown plans
        ...
        DB::beginTransaction();
        try {
            // Temporarily update restaurant count for cost calculation
            $originalRestaurantCount = $tenant->total_restaurants;
            $tenant->total_restaurants = $newRestaurantCount;

            // Calculate upgrade cost (prorated amount)
            $upgradeCost = $tenant->calculateUpgradeCost($newPlan);

            // Reset restaurant count for now
            $tenant->total_restaurants = $originalRestaurantCount;

            // If this is an upgrade (costs more), create a payment record for the difference
            if ($upgradeCost > 0) {
                // Cancel any existing pending payments
                $tenant->subscriptionPayments()
                    ->where('status', SubscriptionPayment::STATUS_PENDING)
                    ->update(['status' => SubscriptionPayment::STATUS_FAILED]);

                // Get current subscription for billing period calculation
                $currentSubscription = $tenant->getCurrentSubscriptionPayment();
                $billingPeriodStart = now();
                $billingPeriodEnd = $currentSubscription ? $currentSubscription->billing_period_end : now()->addMonth();

                // Create payment record for upgrade cost
                $payment = SubscriptionPayment::create([
                    'tenant_id' => $tenant->id,
                    'subscription_plan' => $newPlan,
                    'restaurant_count' => $newRestaurantCount,
                    'base_amount' => $newLimits['base_fee'],
                    'per_restaurant_amount' => $newLimits['per_restaurant_fee'],
                    'total_amount' => $upgradeCost, // Only charge the difference
                    'billing_period_start' => $billingPeriodStart,
                    'billing_period_end' => $billingPeriodEnd,
                    'payment_method' => SubscriptionPayment::PAYMENT_METHOD_CARD,
                    'payment_gateway' => 'stripe',
                    'status' => SubscriptionPayment::STATUS_PENDING,
                    'due_date' => now()->addDays(7),
                ]);

                // Update tenant plan (but keep it pending until payment)
                $tenant->update([
                    'subscription_plan' => $newPlan,
                ...
                return redirect()->route('admin.tenant.payment.checkout')
                    ...
                    ->with('upgrade_info', [
                        ...
                        'is_upgrade' => true,
                        'pricing_window' => $tenant->getPricingWindowInfo(),
                    ]);
            } else {
                // This is a downgrade or same-cost change, no additional payment required
                $tenant->update([
                    'subscription_plan' => $newPlan,
                    'monthly_base_fee' => $newLimits['base_fee'],
                    'per_restaurant_fee' => $newLimits['per_restaurant_fee'],
